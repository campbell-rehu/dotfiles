    ...
    vim.api.nvim_create_autocmd("LspAttach", {
      ...
      callback = function(ev)
        local generateLSPKeymap = function(mode, keymap, command, description, buffer)
          helpers.generateKeymap(mode, keymap, command, { buffer = buffer, silent = true, desc = description })
        end
        -- setup keymaps
        ...
        generateLSPKeymap("n", "gi", "<cmd>Telescope lsp_implementations<CR>", "Show LSP implementations", ev.buf)
        generateLSPKeymap("n", "gt", "<cmd>Telescope lsp_type_definitions<CR>", "Show LSP type definitions", ev.buf)
        generateLSPKeymap({ "n", "v" }, "<leader>ca", vim.lsp.buf.code_action, "See available code actions", ev.buf)
        generateLSPKeymap("n", "<leader>rn", vim.lsp.buf.rename, "Smart rename", ev.buf)
        generateLSPKeymap("n", "<leader>D", "<cmd>Telescope diagnostics bufnr=0<CR>", "Show buffer diagnostics", ev.buf)
